/*
7. Write a Java program to create a base class Animal with methods move() and makeSound().
Create two subclasses Bird and Panthera. Override the move() method in each subclass to describe
how each animal moves. Also, override the makeSound() method in each subclass to make a
specific sound for each animal.
*/

public class Panthera extends Animal {



    @Override
    public void move(){

        System.out.println(this.getClass().getName() + " on the move");
    }

    @Override
    public void makeSound(){


        System.out.println("Dont even know what a panthera is");
    }

}